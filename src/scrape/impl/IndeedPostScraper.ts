...
export class IndeedPostScraper extends PostScraper {
    ...
    async clearPopup () {
        ...
        if ( found > 0 )
            await this.page.click( '[aria-label="Close"]' ).catch( () => {
                ...
                if ( this.page )
                    this.page.screenshot({ path: path.join( __dirname, '../../../dist', 'NEXT-failure-' + new Date().getTime() + '.png' ) });
            });
    }
    ...
    async nextPage (): Promise<void> {
        ...
        await Promise.all( [ this.page.click( '[aria-label="Next"] svg' ), this.page.waitForNavigation() ] ).catch( ( err ) => { console.error( err ) });
        this.currentPage++;
