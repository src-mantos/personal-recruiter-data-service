...
DataRouter.post(
    ...
    async ( req: Request, res: Response ) => {
        ...
        if(query.keywords !== undefined && query.keywords !== ""){
            ...
            results = await postDao.getAllPosts( query.filters );
        }
...
/**
 * POST /v1/post/updatePost/{id}
 * @summary Updates post data entry records
 * @tags post
 */
PostRouter.post( '/updatePost/:id', async ( req: Request, res: Response ) => {
    const update = req.body as PostData;
    const result = await postDao.setPostById( update );
    res.json( result );
});
/**
 * GET /v1/post/updatePost/{id}
 * @summary lookup post data?
 * @tags post
 */
PostRouter.get( '/updatePost/:id', async ( req: Request, res: Response ) => {
    const update = req.body as ScrapeRequest;
    const result = await postDao.getPostById( update._id );
    res.json( result );
});
